...
- Add support for `einx.dot` with [mlx](https://github.com/ml-explore/mlx) (mlx now supports [einsum](https://ml-explore.github.io/mlx/build/html/python/_autosummary/mlx.core.einsum.html#mlx.core.einsum)).
- Add support for `einx.vmap`, `einx.{index|get_at|set_at|...}` with [mlx](https://github.com/ml-explore/mlx). Both use [mx.vmap](https://ml-explore.github.io/mlx/build/html/python/_autosummary/mlx.core.vmap.html) internally which is in development and does not fully support all operations that can be expressed in einx. If this causes an error during an einx call, the error can be reproduced without einx by copying and calling the Python function generated by einx directly (see [Just-in-time compilation](https://einx.readthedocs.io/en/stable/more/jit.html)).
